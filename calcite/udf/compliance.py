...
# def unify_rows(schema, rows):
#     """
#     Given a schema (list of {name, type}) and rows (list of lists),
#     return a list of dicts mapping each column name to its value.
#     """
#     names = [col['name'] for col in schema]
#     return [
#         { names[i]: row[i] for i in range(len(names)) }
...
#     for i in range(1, int(max_query_id) + 1):
#         qid        = f'{i:03d}'
#         query_file = os.path.join(query_dir, f'query_{qid}.ppl')
#         exp_file   = os.path.join(query_dir, f'query_{qid}.results')

#         if not os.path.isfile(query_file):
#             report.append({
#                 'query_id':    qid,
#                 'status':      'failed',
#                 'reason':      'missing .ppl file',
#                 'raw_response':''
#             })
#             continue

#         # load & sanitize the PPL
#         raw_query = open(query_file).read()
#         query     = re.sub(r'\b\w+\.default\.', '', raw_query).strip()
...
#         # if expected has no schema, fail immediately
#         if 'schema' not in expected:
#             report.append({
#                 'query_id':    qid,
#                 'status':      'failed',
#                 'reason':      f'missing schema in expected-results: {json.dumps(expected)}',
#                 'raw_response': raw_resp
#             })
#             continue        
...
#         actual_dicts   = unify_rows(actual_schema, data['datarows'])
#         expected_dicts = unify_rows(expected_schema, expected['datarows'])

#         # 2. Use Counter of JSON strings for order-insensitive comparison
#         act_cnt = Counter(json.dumps(r, sort_keys=True) for r in actual_dicts)
#         exp_cnt = Counter(json.dumps(r, sort_keys=True) for r in expected_dicts)
...
#             unexpected = [json.loads(s) for s in (act_cnt - exp_cnt).elements()]
#             status = 'failed'
#             reason = json.dumps({'missing': missing, 'unexpected': unexpected}, indent=2)
...
#     write_html_report(report, report_path='report.html')
